...
const sql2 = new SQLite('./databases/guild.sqlite')
        ...
        if(interaction.guild) {
			...
			if(leaderboard === 'top5') {
				...
				for(const data of top5) {
					...
					if(!user) {
						embed.addField('\u200b', i.toString()+') **'+user.user.username+'** with over **'+nFormatter(data.points, 2)+' XP** and at **Level '+nFormatter(data.level, 0)+'**');
					} else {
			...
			if(leaderboard === 'top10') {
				...
				for(const data of top10) {
					...
					if(i <= 6) {
						...
						if(!user) {
							embed.addField('\u200b', i.toString()+') **'+user.user.username+'** at **Level '+nFormatter(data.level, 0)+'**');
						} else {
